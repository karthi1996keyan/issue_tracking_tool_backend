                    ...
                    let reporterNotify=new notification(
                        {
                            ...
                            description:`${issueDetails.title} : has been posted successfully`,
 ...
 let getSingleIssue=(req,res)=>
 {
     ...
     .exec(
         (err,issue)=>
         {
             ...
             {
               let apiresponse=responseLib.generate(false,'Issue found successfully',200,issue);
               res.send(apiresponse);
             }
         }
     )
 ...
 let addComment=(req,res)=>
 {
     ...
     newComment.save(
         (err,success)=>
         {
             ...
             notification.updateMany({issueId:req.body.issueId},options,(req,res)=>
            {
                if(err)
                {
                    console.log('err');
 ...
 */
 ...
 let countUpdate=(req,res)=>
 {
     ...
     notification.updateMany({userId:req.body.userId},options)
     .exec((err,success)=>
     {
         if(err)
         {
 ...
 let searchIssue = async (req, res) => {
    ...
    if(checkLib.isEmpty( req.query.arg))
    {
        
        loggerLib.captureError('No argument entered for search','issueController:searchIssue',10);
        let apiresponse=responseLib.generate(true,'No argument entered for search',500,null);
        res.send(apiresponse);

    }

    issueModel.find({$text:{$search:req.query.arg}}).limit(10).skip(parseInt(req.query.skip))
    .exec(
        (err,response)=>
        {
         ...
         if(err)
         {
            loggerLib.captureError(err+'error while retreving data','issueController:searchIssue',10);
            let apiresponse=responseLib.generate(true,'Error while retreiving data',500,null);
            res.send(apiresponse);
         }
         else if(checkLib.isEmpty(response))
         {
            loggerLib.captureError('no data present by this search string','issueController:searchIssue',10);
            let apiresponse=responseLib.generate(true,'no data present by this search string',404,null);
            res.send(apiresponse);
         }
         else
         {
            let apiresponse=responseLib.generate(true,'data present by this search string',200,response);
            res.send(apiresponse);
         }
        }
    )
